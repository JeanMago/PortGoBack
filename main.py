...
class SchoolRankSchema(BaseModel):
    rank: int
    school: str
    total_score: int
...
def get_db_ranking(db: Session):
    return db.query(User).order_by(User.score.desc()).all()

def get_db_ranking_by_school(db: Session, school: str):
    return db.query(User).filter(User.school.ilike(f"%{school}%")).order_by(User.score.desc()).all()

def get_db_questions_by_level(db: Session, level: str, limit: int = 10):
    return db.query(Question).filter(Question.level == level).limit(limit).all()
...
# CORREÇÃO DE CORS
origins = ["https://jeanmago.github.io/PortGO/#/", "https://jeanmago.github.io"]
...
def get_general_ranking(db: Session = Depends(get_db)):
    return get_db_ranking(db)

@app.get("/ranking/school/{school_name}", response_model=List[UserSchema], tags=["Ranking"])
def get_school_ranking(school_name: str, db: Session = Depends(get_db)):
    return get_db_ranking_by_school(db, school=school_name)
    
@app.get("/ranking/schools", response_model=List[SchoolRankSchema], tags=["Ranking"])
def get_schools_leaderboard(db: Session = Depends(get_db)):
    return get_schools_ranking(db)
...
def read_questions_by_level(level: str, limit: int = 10, db: Session = Depends(get_db)):
    return get_db_questions_by_level(db, level, limit)